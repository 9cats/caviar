...
/* 请求参数摘要 */
function verifyParams(params: any) {
...
export class ChaoXing {
  ...
  async getToken(roomId: String): Promise<String> {
    return await this.agent
      ...
      .then((res) => {
        let index = res.text.indexOf("token = '") + 9;
  ...
  async sbumit(
    ...
    /* 获取图形验证信息 */
    let captcha = await this.agent
      .get("http://chaoxing_slidecaptcha_verify:8888/validate/pop")
      .then(async (res) => {
        if (res.body.success) {
